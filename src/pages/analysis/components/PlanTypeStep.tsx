  ...
  return (
    <div className="bg-white rounded-lg p-4 sm:p-8 md:p-12">
      <div className="mb-6 sm:mb-12 text-center">
        <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold text-cream-900 mb-2 sm:mb-4">選擇保險規劃類型</h2>
        <p className="text-base sm:text-lg md:text-xl text-cream-600">請選擇您需要的保險規劃方案</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-8 max-w-4xl mx-auto mb-6 sm:mb-12">
        ...
        <button
          ...
          className={`p-4 sm:p-8 rounded-lg border-2 transition-all cursor-pointer ${
            selectedType === 'adult'
        ...
        >
          <div className="flex flex-col items-center text-center">
            <div className={`w-16 h-16 sm:w-24 sm:h-24 rounded-full flex items-center justify-center mb-3 sm:mb-6 overflow-hidden ${
              selectedType === 'adult' ? 'bg-teal-100' : 'bg-cream-200'
            }`}>
              {adultIcon ? (
                <img src={adultIcon} alt="成人保險" className="w-full h-full object-cover" />
              ) : (
                <i className={`ri-user-line text-3xl sm:text-5xl ${
                  selectedType === 'adult' ? 'text-teal-600' : 'text-cream-500'
                }`}></i>
              )}
            </div>
            <h3 className="text-lg sm:text-2xl font-bold text-cream-900 mb-2 sm:mb-3">成人保險規劃</h3>
            <p className="text-sm sm:text-base text-cream-600 mb-2 sm:mb-4">
              為成年人量身打造的保險方案，涵蓋醫療、癌症、意外等全方位保障
            </p>
            <div className="flex items-center justify-center space-x-2 text-xs sm:text-sm text-cream-500">
        ...
        >
          <div className="flex flex-col items-center text-center">
            <div className={`w-16 h-16 sm:w-24 sm:h-24 rounded-full flex items-center justify-center mb-3 sm:mb-6 overflow-hidden ${
              selectedType === 'child' ? 'bg-teal-100' : 'bg-cream-200'
            }`}>
              {childIcon ? (
                <img src={childIcon} alt="幼兒保險" className="w-full h-full object-cover" />
              ) : (
                <i className={`ri-parent-line text-3xl sm:text-5xl ${
                  selectedType === 'child' ? 'text-teal-600' : 'text-cream-500'
                }`}></i>
              )}
            </div>
            <h3 className="text-lg sm:text-2xl font-bold text-cream-900 mb-2 sm:mb-3">幼兒保險規劃</h3>
            <p className="text-sm sm:text-base text-cream-600 mb-2 sm:mb-4">
